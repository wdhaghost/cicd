...
@section('title', 'NWS - Modifier un prêt')

@section('content')
    <div class="container mx-auto mt-5">
        <h2 class="b-4">Emprunter du matériel</h2>

        <form action="{{ route('loans.update') }}" method="post">
            @csrf

            <div class="row mb-4">
                <label for="loan_date" class="form-label">Date d'emprunt:</label>
                <input type="date" name="loan_date" id="loan_date" class="form-input w-50 mt-1 " value="{{ old('loan_date',$loan->loan_date) }}" required>
                @error('loan_date')
            ...
            <div class="row mb-4">
                ...
                <select name="student_id" id="student_id" class="form-select w-full" required>
                    <option value="{{old("student_id",$student->id)}}" disabled selected>Sélectionnez un étudiant</option>
            ...
            <div class="row mb-4">
                ...
                <select name="equipment_id" id="equipment_id" class="form-select w-full" required>
                    <option value="{{old('equipment_id',$equipment->id)}}" disabled selected>Sélectionnez un équipement</option>
